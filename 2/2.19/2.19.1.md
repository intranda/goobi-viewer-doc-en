...
</table>Below an `<param>` element, `<replace />` rules can be defined. The attributes `string`, `char` or `regex` can be used to define values that are extracted from the metadata and then replaced by the value specified in the replace block. 
