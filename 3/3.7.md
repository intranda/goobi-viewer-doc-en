...
This allows geocoordinates of points and polygons from [GML](https://de.wikipedia.org/wiki/Geography_Markup_Language) or `mods:coordinates` to be written as [geoJSON](https://de.wikipedia.org/wiki/GeoJSON) instead of in the original format. 
...
Optionally the attribute `addSearchField="true"` can be defined. In this case, the `WKT_COORDS` field is also written, which contains the coordinates of the point or polygon in [WKT](https://de.wikipedia.org/wiki/Simple_Feature_Access#Well-known_Text) format.
